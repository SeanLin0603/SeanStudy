    ...
    public class QRCodeReader : Reader
    {
        ...
        public Result decode(BinaryBitmap image, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (image == null || image.BlackMatrix == null)
            {
                ...
                return new Result("IMAGE_ERROR", null, null, BarcodeFormat.QR_CODE, Status.NotFound);
            }
            if (hints != null && hints.ContainsKey(DecodeHintType.PURE_BARCODE))
            {
                var bits = extractPureBits(image.BlackMatrix);
                if (bits == null)
                    return new Result("BITS_ERROR", null, null, BarcodeFormat.QR_CODE, Status.NotFound);
            ...
            else
            {
                ...
                if (detectorResult == null)
                    return new Result("DETECTOR_ERROR", null, null, BarcodeFormat.QR_CODE, Status.NotFound);
                decoderResult = decoder.decode(detectorResult.Bits, hints);
                points = detectorResult.Points;
            }
            if (decoderResult == null)
                return new Result("DECODER_ERROR", null, null, BarcodeFormat.QR_CODE, Status.NotFound);
            ...
            var result = new Result(decoderResult.Text, decoderResult.RawBytes, points, BarcodeFormat.QR_CODE, decoderResult.Status);
