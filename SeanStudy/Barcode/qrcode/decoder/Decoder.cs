...
namespace ZXing.QrCode.Internal
{
    ...
    public sealed class Decoder
    {
        ...
        public DecoderResult decode(BitMatrix bits, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (parser == null)
                return new DecoderResult(null, "PARSER_ERROR", null, null, Status.FormatError);

            var result = decode(parser, hints);
            if (result == null || result.Status != Status.NoError)
            {
                ...
                if (version == null)
                    return new DecoderResult(null, "VERSION_ERROR", null, null, Status.FormatError);

                // Preemptively read the format information.
                var formatinfo = parser.readFormatInformation();
                if (formatinfo == null)
                    return new DecoderResult(null, "FORMAT_INFO_ERROR", null, null, Status.FormatError);
        ...
        private DecoderResult decode(BitMatrixParser parser, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (codewords == null)
                return new DecoderResult(null, "CODEWORDS_ERROR", null, null, Status.FormatError);
            ...
            foreach (var dataBlock in dataBlocks)
            {
                ...
                if (!correctErrors(codewordBytes, numDataCodewords))
                    return null;
                    //return new DecoderResult(null, "CORRECTION_ERROR", null, null, Status.ChecksumError);
